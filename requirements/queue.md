...
## Overview
The queue is what plays next. It is more immediate than a playlist — a way to steer the next hour without rearranging anything permanent (`playlists.md`).
...
- **The Manual Queue always plays first**, ahead of everything in the context. **Queue Next** inserts at its front, **Queue Last** at its end (`conventions.md` §2).
...
- **A queue can mix owned and external content.** Where a plugin provides an external source, its tracks queue alongside the user's own and play in the same run, marked as external (`plugins.md` §10.2).
...
- **History belongs to the queue, not the account.** It reflects this queue only, and is distinct from listening history (`analytics.md`), which records every play on every device.
...
- **A shuffled order is fixed once generated.** It does not reshuffle as the user scrolls Up Next, and it is the same order on every device (§7) and offline (`offline.md` §6).
- **Repeat All is the one exception, and it does not reshuffle.** Restarting a shuffled queue replays the same order it already generated, exactly as repeating an album replays the same running order (§5). A user who wants a different order asks for one by toggling shuffle.
...
- **Repeat** cycles **Off → All → One**. *All* restarts the queue when it ends, in the order it was already playing in — shuffled or not (§4); *One* repeats the current track until the user changes it.
- **Endless play** decides what happens when a queue ends with repeat off: playback either **stops**, or **continues with similar tracks** (`recommendations.md`). A user who starts an album and walks away can have the music keep going, or not.
...
- **The Manual Queue's fate is the user's choice.** A preference decides whether manually queued tracks **carry over** into a new context or are **parked** with the session they belong to (`users.md` §6). They carry over by default: the user asked for them explicitly.
...
- **Changes appear immediately.** Reordering on a phone is visible on a desktop without refreshing (`realtime.md`).
- **Playback moves between devices** without rebuilding the queue (`realtime.md` §3).

---

## 8. Resilience

- **Size is irrelevant.** Opening, shuffling, or reordering a 500,000-track context is as immediate as a 10-track one (`performance.md`).
...
- **Missing and undownloaded tracks are skipped in place** (`conventions.md` §6). Skipping never renumbers or rebuilds the queue.
