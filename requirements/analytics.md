...
## Overview
Jewelcase records what a user listens to, so they can look back on it and so the system can learn what they like (`recommendations.md`).
...
Listening data is **personal and private** (`conventions.md` §1, `users.md` §7).
...
- **The context** it was played from, as defined in `queue.md` — including radio and endless play, which are contexts like any other.
- **The device** it played on (`users.md` §4).
...
- **Local playback is recorded identically** to streamed playback (`offline.md` §4).
- **External content is recorded and marked as external** (`plugins.md` §10.2). It is still listening, and leaving it out would make a user's own history wrong — but it never influences recommendations (`recommendations.md` §1).
...
- **These, not play counts, drive recommendations.** Because there is no explicit way to say "I like this" (§7), completion is the primary evidence of enjoyment, and treating a high play count as approval without it would actively mislead.
...
- **Distinct from a queue's History** (`queue.md` §2), which covers only the current session. This is the permanent record.
...
- **Top tracks, albums, artists, and genres** (`tags.md`).
...
- **Ranking accounts for completion** (§2), so a skipped track does not out-rank a loved one on play count alone.
...
- **Plays are recorded at the time they happened**, not the time they synced. A week offline lands on the right days (`offline.md` §8).
...
- **Recommendations rest entirely on implicit signals** (§2), which makes completion data more important than it would otherwise be.
...
- **History survives its tracks.** When a file is purged from the library, past plays remain readable, showing the correct title and artist, marked unavailable (`scanning.md` §8.1).
- **Statistics never shift retroactively.** Cleaning up missing files does not change what a user listened to last year.
- **Retagging does not rewrite history.** Identity follows content (`scanning.md` §6), so a corrected tag updates how a past play is displayed without altering that it happened.

---

## 9. Privacy and Control

- **Private between users** (`users.md` §7). Admins can view any account's history and statistics, and can never modify them (`users.md` §8).
...
- **Exportable** in a portable, documented format, by the user or by an admin on their behalf (`users.md` §9).
