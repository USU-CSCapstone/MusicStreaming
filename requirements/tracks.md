...
Every field comes from the file's tags. Jewelcase adds no metadata of its own and offers no way to edit what it read. Discovery and parsing belong to `scanning.md`; this file describes what a track *is*.

Shared behavior — personal data, actions, presentation, availability, multi-artist display — follows `conventions.md`.

---

## 1. Identity

A track's identity follows its **content** — not its path and not its tags (`scanning.md` §6).
...
- **Title**
- **Artists** — an ordered list (§3)
- **Album** — with its own artists (`albums.md`)
...
A track's artists are distinct from its **album's** artists. The relationship between the two determines ownership versus feature credit, per `artists.md` §2.

Parsing follows `artists.md` §1; display follows `conventions.md` §7.
...
- **The file's path is recorded but not general-purpose information.** It is needed to serve the file and useful to an admin diagnosing a scan problem; it is not part of what a listener browses, and is not sent to clients that do not need it (`general.md` §3.7).
...
Resolution from disk follows `scanning.md` §3.4. Lyrics are never authored, edited, or fetched by the core.
...
- **Genres** preserved as multiple values (`tags.md`).
