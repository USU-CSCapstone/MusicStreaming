...
## Overview
Offline is a first-class mode, not a degraded fallback (`general.md` §1). It follows from **offline parity** (`general.md` §3.5): the same inputs produce the same queue order, the same sort, and the same navigation with or without a network.

Two differences are permissible, and only these: **whether a track's audio can play**, and **what the device has to search** — a smaller corpus, never a weaker search (`search.md` §2.1). Everything else must be identical.
...
**Every library a user can access is browsable, searchable, and queueable on their device without downloading anything** — including tracks whose audio is not present. This is the design target and the normal case; §1.2 is the only exception.
...
### 1.1 Footprint
The device footprint is **bounded well below what the music itself would cost** — even at the 500,000-track target, comparable to a handful of downloaded songs (`performance.md` §6).
...
- **Full-resolution imagery is cached as it is seen**, within a bounded cache, and is always present for downloads (§2).
- **Lyrics arrive with downloads**, on demand otherwise — too large to carry wholesale, and of no use without the audio. **Downloaded lyrics are indexed and searchable on the device**, so a user who has downloaded everything can search lyrics offline exactly as they would online (`search.md` §2.1).
- **Waveforms travel the same way as lyrics** (`playback.md` §3): bundled with a download, fetched on demand for a streamed track, cached once seen. They are per-track data useful only while that track is playing, and at 500,000 tracks would cost more than the whole catalog budget — so they are never pre-synced wholesale. **Every downloaded track carries its waveform**, so offline playback looks identical to online.
- **Similarity data rides along with the catalog**, compact enough to be part of it rather than a second index, so endless play keeps working offline (`recommendations.md` §10).

Imagery a user has seen or downloaded is real; the rest is a deliberate placeholder (`conventions.md` §5), never a blank grid.

### 1.2 When the Catalog Does Not Fit
The footprint budget (§1.1) is set so this should not happen. Where a device genuinely cannot carry the full catalog anyway, the guarantee that survives is: **anything downloaded remains completely self-sufficient** (§2), as do the user's playlists, queues, and history.

Any shortfall is a **bounded and stated** scope — the user is told what their device holds, and search says what it covered (`search.md` §2.2). What must never happen is the device answering a question *less well* than the server would; a smaller catalog is acceptable, a weaker search is not (`search.md` §2.1).
...
- **Partial containers are honest.** An artist with 1 of 12 albums downloaded says so (`albums.md` §4). Partial is normal, never an error.

---

## 3. What Can Be Downloaded

Any entity, wherever the Download action appears (`conventions.md` §2): **a track, album, artist, playlist, or entire library.**

**A download is a standing instruction, not a snapshot.** Tracks added to a downloaded playlist download automatically and removed ones free their space (`playlists.md` §10); tracks that scanning adds to a downloaded album, artist, or library are downloaded too.
...
Quality is set per device, separately for unmetered connections, metered connections, and downloads (`users.md` §6).
...
- **Local playback costs the server nothing**, consuming no concurrent stream (`performance.md` §1).
- **The source is invisible downstream.** Plays, history, statistics, and remote control behave identically either way (`analytics.md`, `realtime.md`).
...
- **The catalog and image cache sit outside the budget** — the user did not choose them and cannot remove them without breaking offline browsing. Their cost is disclosed, and keeping it small is the project's responsibility (§1.1).

**Changing the download quality (§4) applies to new downloads only.** Re-fetching existing ones at the new quality is an explicit user action, never an automatic re-download.
...
- **Browse, search, sort, and filter** across the full library, with search matching and ranking exactly as the server does (`search.md` §2.1).
- **Build and manipulate queues**, including shuffle, over any context (`queue.md` §4) — the same context, sorting, and filters produce the same sequence they would online.
- **Create and edit playlists**, reconciling on reconnect (§8).
- **Play what is downloaded**, with plays and listen time recorded locally (§8).
- **"Not on this device" and "gone from the server" are different states**, presented differently, and both skipped in place during playback (`conventions.md` §6).
...
- **Playlist edits follow the same rule as any other edit** (`playlists.md` §5): applied if the playlist has not changed meanwhile, refused with a plain explanation if it has.
- **Plays reconcile per `analytics.md` §6** — timestamped when they happened, never when they synced, and never counted twice however many times a device reconnects.
...
- **A downloaded track whose file leaves the server keeps playing on that device**, marked as no longer in the library. It stops counting against server retention (`scanning.md` §8).
- **Retagging updates metadata, not audio.** Identity follows content (`scanning.md` §6), so a retagged or moved file is the same download.
- **Replaced audio re-downloads.**
- **Losing access to a library removes its downloads from the device** — access is the boundary (`general.md` §3.6). Playlists, history, and statistics survive and return if access is restored (`users.md` §5).
- **Signing a device out removes its downloads**, as does deleting the account (`users.md` §9).
- **A password change is not that case.** Changing a password signs other devices out (`users.md` §3), but those devices **keep their downloads**, usable again the moment the same account signs back in. The same holds for an admin-initiated reset. A user securing their account must not pay for it by re-downloading their library over cellular.
...
- **Downloads yield to listening** (`performance.md`).

---

## 11. Platform Parity

**Offline audio works on the responsive web app and the Android app alike.** iOS reaches Jewelcase only through the PWA (`general.md` §4), so anything less means no offline listening on iPhone at all.
