...
That openness has exactly one price, and it is not negotiable: **a plugin may do anything, but it may never make Jewelcase slow.** The system is unbounded in *kind* and bounded in *cost* (§2).
...
**If the server knows it or does it, a plugin can reach it.** The useful shorthand: *anything a third-party client can do through the public API (`general.md` §4), a plugin can do* — plus the server-side hooks that let it act on its own rather than waiting to be asked.

Things people will build early, offered as a starting point and explicitly **not** the boundary:

- **Metadata and artwork enrichment** — artist images, biographies, album art, corrected tags (§7).
- **Lyrics**, timed or plain, for tracks that have none (`tracks.md`).
- **Scrobbling** to Last.fm, ListenBrainz, and anything similar (`analytics.md` §10), built on events (§8).
- **External content sources** — music the user does not own, browsable and playable alongside music they do (§10).
- **Acquisition** — placing new music into the library, which the scanner then treats like anything else (`general.md` §2).
- **Automation and reporting** — scheduled work, health checks, exports, backups, notifications.
- **Interface surfaces** of their own (§9).
...
- **Core results never wait on plugin results.** A search returns the library's matches within its own budget (`performance.md` §3); external sections fill in as their sources answer.
...
- **Background plugin work yields to listeners** (`performance.md` §7). Enrichment surrenders resources to playback, always.
...
This qualifies a promise made in `general.md` §1, and is stated plainly rather than buried:

> **The core never touches your files. A plugin you install can do anything to them.**

That is the trade, made knowingly at install time (§5). Restricting plugins to sidecar files would rule out the two things they are most wanted for: correcting bad tags in place, and bringing new music in.

- **Everything a plugin writes is ingested by the scanner** (`general.md` §3.2). There is no side channel into the database, so a plugin-fetched biography and a hand-placed one are indistinguishable.
...
- **Only admins install plugins** (`users.md` §8). Ordinary users can neither add nor enable one.
...
- **Enabled per library, independently** (`libraries.md`). A plugin may serve one library and never see another.
- **Disabling is immediate and complete.** A disabled plugin runs nothing, sees nothing, and reaches nothing.
- **Uninstalling leaves the library alone.** Files a plugin wrote are ordinary files and stay; content it merely *sourced* disappears with it (§10).
...
- **Results appear as the scanner finds them** (`scanning.md` §5), with no separate refresh for the user to perform.
...
- **Play events carry enough detail to be acted on accurately**, scrobbling included (`analytics.md` §10).
...
- **Events never block anything** (§2). Playback, scanning, and every user action complete regardless of whether a plugin is listening, slow, or broken.
...
- **Plugin surfaces are real surfaces**, navigable and actionable like anything the core provides (`conventions.md` §2).
...
- **They fill in rather than block** (§2.2). A plugin's section may take as long as its source takes; the page around it renders immediately regardless.
- **A failing plugin surface degrades locally** — its own section says something useful and the rest of the screen is untouched (§11).
...
**External content never enters the library, and is never scanned, indexed, or owned.** The scanner remains the only ingestion path (`general.md` §3.2) precisely because this content does not use it — it is not ingested at all, only surfaced.
...
- **It is scoped to a library** like everything else (`general.md` §3.6), appearing only where the plugin is enabled.
...
- **Searchable**, in its own clearly labelled section of results (`search.md` §1) — never mixed into the sections describing what the user owns.
- **Browsable** through whatever structure the source offers — artists, albums, playlists.
- **Queueable anywhere**, alongside owned tracks in the same queue (`queue.md` §1). A mixed queue is the entire point.
- **Playable with the same controls** — the same player, the same transport, the same Shift behaviour across devices (`realtime.md`).
- **Recorded in listening history and statistics**, marked as external (`analytics.md` §1). It is still listening, and omitting it would make a user's own history wrong.
- **Addable to playlists**, marked as external. If the source goes away the entry behaves exactly like a missing track — held in place, unplayable, explained (`playlists.md` §3).

### 10.3 Its Limits

- **It cannot be downloaded** and does not work offline, presented like any other track whose audio cannot play (`conventions.md` §6).
- **It never drives recommendations** (`recommendations.md` §1), so a guest's choices in the car do not reshape what the owner is offered for weeks afterward.
...
- **Cross the account boundary.** Listening history, playlists, and queues are private between users (`users.md` §7).
- **Cross the library boundary** into libraries it is not enabled for (`general.md` §3.6).
