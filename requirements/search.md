...
**The server searches when it is reachable; the device searches when it is not.** Preferring the server is an efficiency choice — it spares the device the work and the battery, and it always holds the complete library. It is not a quality choice. **The two searches are the same search**, and the device's is not a lesser version of it (§2).
...
Nothing else in the library. File paths and folder names are **not** searched: the path carries no meaning in Jewelcase (`scanning.md` §2), and matching on it would produce results a user cannot explain.

**External sources, where a plugin provides one, are searched too** — returned in their own clearly labelled section, never mixed into the sections describing what the user owns (`plugins.md` §10.2).
...
- **A device still syncing is never worse off.** Search covers the whole library from the first sign-in, long before any catalog sync finishes (`offline.md` §1).
- **The device searches when the server is not.** It covers the catalog it holds and the lyrics of tracks it has downloaded (`offline.md` §1.1, §2).
...
- **Capability is identical.** The device does the same fuzzy matching, typo tolerance, diacritic folding, word-order independence, and exact-match precedence as the server (§3). No simplification, no prefix-only matching, no "good enough for offline".
- **Ranking is identical.** Same top result, same section ordering, same tie-breaking (§4). Given the same corpus, the two produce the same ordered list.
- **Presentation is identical.** Same sections, same lyric context lines, same actions in place (§4).
...
- **Leading articles do not matter.** `beatles` finds The Beatles (`conventions.md` §4).
...
- **Each section expands to a full list**, sortable per `conventions.md` §4.
- **Every result is actionable in place** — playable, queueable, addable to a playlist — without opening it first (`conventions.md` §2).
...
- **Switching library switches the search.** Users move between libraries freely (`users.md` §5), and search follows without being told to.
- **Cross-library search is supported but not surfaced in v1.** The capability exists for later versions and for third-party clients (`general.md` §4). The shipped apps keep to one library, because merged results raise "which library is this from?" on every row.
- **Never beyond access.** Content a user cannot reach is indistinguishable from content that does not exist (`users.md` §10).
- **Narrowable to a container** — within an artist, an album, or a playlist, without leaving it (`playlists.md` §7).
- **Filterable by type**, so someone looking only for albums is not shown tracks.
- **Composable with filters** — genre, year, and listening data (`tags.md` §7).
...
- **Private to the user** (`users.md` §7), and kept only for a recent window.
...
- **Results within 50 ms at full scale** (`performance.md` §3), measured on both verification machines — and **on the device, against the catalog it holds** (`performance.md` §6). Client search meets the same budget as server search; parity of capability (§2.1) is worth nothing if the device takes a second to deliver it.
...
- **A slow or unreachable server hands off rather than hanging.** The device answers instead, to the same standard (§2.1), and no search ever ends in a spinner that never resolves.
