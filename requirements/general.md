...
This file holds only what the rest of the set derives from: the goals, the boundaries, the principles, and the platforms. Everything else has its own file (`CLAUDE.md`).
...
- **Be genuinely extensible.** The core serves a library and little else. Anything beyond that is a plugin's to build, with no list of permitted capabilities and no need to wait for a core release (`plugins.md`).
- **Work offline as a first-class mode**, not a degraded fallback (`offline.md`).
- **Be pleasant to self-host.** Deployment, upgrades, and backups approachable for a homelab, not just a professional operator (`deployment.md`).

The rules these translate into are §3.
...
Narrower exclusions are stated where they bite: casting protocols in `playback.md` §9, explicit taste signals in `analytics.md` §7, user-applied tags in `tags.md` §1.
...
The one qualification is plugins, which write to the library without restriction (§3.2, `plugins.md` §3). The core's promise is unchanged: uninstall Jewelcase with no plugins installed and the collection is exactly as it was.
...
Music a plugin merely *surfaces* rather than places is not library content and is never ingested at all (`plugins.md` §10.1).
...
The figures the project verifies and publishes are benchmarks, not ceilings (`performance.md` §1–§2).
...
2. **What a device has to search.** Search offline covers the catalog the device holds and the lyrics it has downloaded — a smaller *corpus*, never a smaller *capability* (`search.md` §2).
...
Storage paths are the case worth naming: a path carries no meaning in Jewelcase (`scanning.md` §2) and discloses the shape of the host's filesystem, so it is exposed deliberately and narrowly rather than by default (`tracks.md` §4).
...
**The public API is a commitment, not a byproduct.** It is what third-party clients build against and what defines the reach of a plugin (`plugins.md` §1).
