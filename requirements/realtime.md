...
- **It is the same device list users already have** ([`users.md` §4](users.md#4-sessions--devices)): the list in settings and the list you send music to are one list, with one name per device.
...
**Exactly one device plays per account at any moment** ([`playback.md` §6](playback.md#6-playback-state)).
...
- **The single queue depends on this** ([`queue.md` §7](queue.md#7-across-devices)). Two devices playing different things at once would make "where am I in this queue" unanswerable, so the constraint is deliberate rather than a limitation.
...
- **Remote control.** Drive the playing device from another: play and pause, skip, seek, shuffle, repeat, volume, and every queue edit ([`queue.md` §3](queue.md#3-editing)).
...
|---|---|
| **Playback** | What is playing, position, paused or not, shuffle, repeat ([`playback.md` §6](playback.md#6-playback-state)) |
| **Queue** | Contents, order, and position ([`queue.md` §7](queue.md#7-across-devices)) |
| **Playlists** | Every edit, as it happens ([`playlists.md` §5](playlists.md#5-concurrent-edits)) |
| **Library** | Music added, changed, or removed by scanning ([`libraries.md` §5](libraries.md#5-staying-current)) |
| **Personal data** | Play counts, listening history, download state ([`conventions.md` §1](conventions.md#1-personal-data)) |
| **Admin views** | Scan progress and problems, current listeners and streams, capacity ([`users.md` §8](users.md#8-admin-capabilities--visibility), [`performance.md` §7](performance.md#7-under-saturation)) |
...
- **Control feels immediate.** A command issued on one device takes effect on another within **300 ms** on a healthy connection ([`performance.md` §3](performance.md#3-interaction-budgets)).
- **The controlling device responds instantly**, without waiting for the target — optimistic, then reconciled ([`general.md` §3.4](general.md#34-optimistic-by-default)).
...
- **Sessions survive long absences** ([`users.md` §4](users.md#4-sessions--devices)). A device closed for a month rejoins without signing in again.
...
- **Playlist edits are the exception**, and follow their own rule: an edit applies only against the version the user was looking at ([`playlists.md` §5](playlists.md#5-concurrent-edits)).
...
- **Realtime is scoped to the account.** A client receives events for its own account and its own libraries, never anything else ([`general.md` §3.6](general.md#36-libraries-are-the-isolation-boundary)).
- **Admins observe, never control** ([`users.md` §8](users.md#8-admin-capabilities--visibility)). Admin views update live with who is listening and what is streaming, but no admin can transfer, start, stop, or redirect another user's playback.
- **Offline devices are not a special case.** A device with no connection behaves per [`offline.md`](offline.md), and rejoins by reconciling like any other client ([`offline.md` §8](offline.md#8-reconnection)).
